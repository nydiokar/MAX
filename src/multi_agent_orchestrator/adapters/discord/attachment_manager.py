...
import aiohttp
from multi_agent_orchestrator.types import DiscordAttachment, ProcessedContent
...
class AttachmentManager:
    ...
    async def process_message_content(self, content: str, raw_attachments: List[Dict]) -> ProcessedContent:
        try:
    ...
    async def process_attachment(self, raw_attachment: Dict) -> Optional[DiscordAttachment]:
    ...
    async def _process_text(self, attachment: DiscordAttachment):
        async with aiohttp.ClientSession() as session:
            async with session.get(attachment.url) as response:
                ...
                attachment.description = f"Text content from {attachment.filename}"
        ...
        if hasattr(self.runtime, 'pdf_service'):
            async with aiohttp.ClientSession() as session:
                async with session.get(attachment.url) as response:
                    ...
                    attachment.text_content = text
                    attachment.description = f"Extracted text from PDF: {attachment.filename}"

    async def _process_image(self, attachment: DiscordAttachment):
        if hasattr(self.runtime, 'image_service'):
            description = await self.runtime.image_service.describe_image(attachment.url)
