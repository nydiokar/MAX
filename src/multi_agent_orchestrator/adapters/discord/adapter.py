...
from multi_agent_orchestrator.types import DiscordAdapterConfig, ProcessedContent
from multi_agent_orchestrator.adapters.discord.attachment_manager import AttachmentManager
from multi_agent_orchestrator.adapters.discord.channel_manager import ChannelManager
...
class DiscordAdapter(PlatformAdapter):
    ...
    async def process_message(self, message: Message) -> ProcessedContent:
        ...
        try:
            # Channel validation
            if not self._channel_manager.is_channel_allowed(message.channel.id):
                return None
    ...
    async def _send_response(self, original_message: Message, response: AgentResponse):
            ...
            for attempt in range(self.config.retry_attempts):
                try:
                    ...
                    if attempt == self.config.retry_attempts - 1:
                        self.logger.error(f"Failed to send message after {self.config.retry_attempts} attempts: {str(e)}")
                        await original_message.channel.send("Sorry, I encountered an error sending the response.")
                    else:
    ...
    def _split_content(self, content: str) -> List[str]:
        ...
        for line in content.split('\n'):
            if len(current_chunk) + len(line) + 1 > self.config.max_message_length:
