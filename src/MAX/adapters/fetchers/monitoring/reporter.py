...
class FetcherReporter:
    ...
    async def generate_summary_report(
        ...
        health_history = self.health_monitor.health_history.get(component.name, [])
        relevant_history = [
            h for h in health_history 
            if h.last_check >= start_time
        ]
    ...
    def _calculate_stats(self, history: List[HealthData]) -> Dict[str, float]:
        ...
        return {
            ...
            "success_rate": 100 - (sum(h.error_rate for h in history) / total_records * 100),
            "avg_latency": sum(h.latency_p95 for h in history) / total_records,
            "p95_latency": sorted([h.latency_p95 for h in history])[int(total_records * 0.95)],
            "cache_hit_rate": sum(h.cache_hit_rate for h in history) / total_records,
            "rate_limit_percentage": sum(1 for h in history if h.rate_limit_hits > 0) / total_records * 100,
            "circuit_breaker_percentage": sum(1 for h in history if h.circuit_breaker_trips > 0) / total_records * 100
        }
    ...
    def _get_metrics_summary(self, metrics: FetcherMetrics) -> Dict[str, Any]:
        ...
        return {
            "total_requests": sum(
                v for k, v in metrics.requests._value.items()
            ...
            "latency_stats": {
                "avg": metrics.latency._sum.get() / max(metrics.latency._count.get(), 1),
                "count": metrics.latency._count.get()
            },
            "cache_operations": {
                "hits": sum(
                    v for k, v in metrics.cache_stats._value.items()
                    if k[1] == "hit"
                ),
                "misses": sum(
                    v for k, v in metrics.cache_stats._value.items()
    ...
    def _identify_issues(
        ...
        if stats["rate_limit_percentage"] > 10:
            issues.append("Frequent rate limiting - consider adjusting request rate")
            
        if stats["circuit_breaker_percentage"] > 5:
            issues.append("Circuit breaker tripping frequently - check endpoint stability")
            
        if stats["cache_hit_rate"] < 0.3:
            issues.append("Low cache hit rate - consider adjusting caching strategy")
