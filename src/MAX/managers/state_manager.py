...
class StateManager:
        ...
        while True:
            try:
                ...
                if not health_status["healthy"]:
                    Logger.warning("Storage health check failed", extra=health_status)
                await asyncio.sleep(self.config.state_manager.health_check_interval_seconds)
            except Exception as e:
    ...
    async def check_storage_health(self) -> Dict[str, Any]:
        ...
        if self.vector_storage:
            vector_healthy, vector_details = await self.vector_storage.check_health()
    ...
    async def _persist_system_state(self) -> bool:
        ...
        try:
            ...
            await self.mongo_storage.save_system_state(
                ...
                ttl=self.config.mongodb.ttl_hours * 3600 if self.config.mongodb.ttl_hours else None
            )
            return True
        except Exception as e:
    ...
    async def restore_state_from_storage(self) -> bool:
        ...
        try:
            ...
            state_data = await self.mongo_storage.get_system_state("SYSTEM_STATE")
            if state_data:
