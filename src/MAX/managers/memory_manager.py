...
class MemorySystem:
    ...
    def __init__(
        ...
        self.queues: Dict[DataCategory, asyncio.Queue] = self._init_queues(queue_size)
        self.processors: List[asyncio.Task] = self._init_processors()

        self.logger.info("Memory system initialized")

    def _init_queues(self, queue_size: int) -> Dict[DataCategory, asyncio.Queue]:
    ...
    async def _process_queue(self, category: DataCategory) -> None:
        ...
        while self.running:
            try:
                ...
                self.logger.debug(f"Processing data ID: {data.id} from {category} queue")

                await self._store_data(data)

                queue.task_done()
                self.logger.info(f"Processed data ID: {data.id} from {category} queue")
