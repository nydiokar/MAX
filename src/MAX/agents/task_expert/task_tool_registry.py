...
from MAX.tools.tool_registry import BaseTool, ToolCategory, BaseToolInput, BaseToolOutput
...
class TaskTool(BaseTool):
    ...
    logger: Any = Field(default_factory=lambda: Logger.get_logger(), exclude=True)
...
class TaskToolRegistry:
    ...
    async def execute(self, tool_name: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            Logger.info(f"Tool {tool_name} executed in {execution_time:.2f}s with params: {params}")
            return result
        
        except ValidationError as e:
            execution_time = time.time() - start_time
            Logger.error(f"Tool {tool_name} validation failed after {execution_time:.2f}s: {str(e)}")
            raise
