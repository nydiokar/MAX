...
from MAX.llms.base import AsyncLLMBase  # Adapt this import to your actual codebase
...
class OllamaLLM(AsyncLLMBase):
    ...
    async def _generate_with_fallback(self, messages: List[Dict[str, str]]) -> str:
        ...
        try:
            ...
            self.logger.error(f"Fallback model '{self.config.fallback_model}' also failed: {e}")
            raise
    @classmethod
    def get_model_for_task(cls, task_type: str) -> str:
        ...
        return task_models.get(task_type.lower(), cls.DEFAULT_MODELS["general"])
