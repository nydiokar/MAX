...
class FileReaderInput(BaseToolInput):
    ...
    def validate_file_size(cls, v, values):
        max_size = values.get('max_size_mb', 10.0) * 1024 * 1024  # Convert to bytes
...
class FileReaderTool(BaseTool):
    ...
    description: str = "Reads content from files with size and encoding validation"
    ...
    def _execute(self, file_path: str, encoding: str = 'utf-8', max_size_mb: float = 10.0) -> dict:
        try:
            ...
            try:
                ...
                raise RuntimeError(f"Failed to read file with alternative encoding: {str(e)}")
        except Exception as e:
