...
class ToolSemanticMatcher:
    def __init__(self, config: SemanticMatcherConfig = None):
        ...
        try:
            ...
            self.nlp = None
            print("Warning: spaCy model not found. Using basic string matching instead.")
    ...
    def compute_similarity(self, query: str, candidates: List[str]) -> List[Tuple[str, float]]:
        ...
        if self.nlp is None:
            ...
            for text in candidates:
                ...
                similarity = len(words_query.intersection(words_text)) / max(len(words_query), len(words_text))
