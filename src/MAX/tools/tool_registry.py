...
class BaseTool(BaseModel):
    ...
    version: str = "1.0.0"
    similes: List[str] = Field(default_factory=list, description="Alternative names for the tool")
    ...
    def execute(self, **kwargs) -> BaseToolOutput:
        ...
        try:
            ...
            self._metadata.average_latency = (
                (self._metadata.average_latency * (self._metadata.usage_count - 1) + execution_time)
...
class ToolRegistry:
    ...
    def find_similar_tools(self, query: str, threshold: float = 0.7) -> List[Tuple[str, float]]:
        """Find semantically similar tools"""
        return self._semantic_matcher.find_similar(query, self._tools.values(), threshold)

    def _update_stats(self, tool_name: str, success: bool, execution_time: float) -> None:
        """Update tool usage statistics"""
        stats = self._usage_stats[tool_name]
        stats["calls"] += 1
        stats["success_rate"] = (stats["success_rate"] * (stats["calls"] - 1) + success) / stats["calls"]
        stats["avg_latency"] = (stats["avg_latency"] * (stats["calls"] - 1) + execution_time) / stats["calls"]
