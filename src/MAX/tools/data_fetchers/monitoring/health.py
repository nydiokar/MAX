...
class HealthMonitor:
    ...
    def _calculate_error_rate(self, metrics) -> float:
        total_requests = sum(metrics.requests.labels(status=FetchStatus.SUCCESS.name).values())
        failed_requests = sum(metrics.requests.labels(status=FetchStatus.FAILED.name).values())
    ...
    def _calculate_p95_latency(self, metrics) -> float:
        ...
        return metrics.latency.observe()  # This needs to be implemented based on your metrics structure
