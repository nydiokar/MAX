...
from .exceptions import XAPIError, XAPIAuthError, XAPIResourceNotFound, XAPIInvalidRequest
...
class XApiFetcher(AbstractFetcher[Union[Tweet, User]]):
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.warning(f"Error clearing metrics: {e}")
        ...
        config = FetcherConfig(
            ...
            enable_metrics=XApiFetcher._instance_count == 1  # Only enable metrics for first instance
        )
        
        # Debug logging with timestamp
        logger.info(f"[{datetime.now().isoformat()}] Initializing X API Fetcher (Instance #{XApiFetcher._instance_count})")
        logger.info(f"[{datetime.now().isoformat()}] Bearer Token available: {bool(settings.X_BEARER_TOKEN)}")
        logger.info(f"[{datetime.now().isoformat()}] API Key available: {bool(settings.X_API_KEY)}")
        logger.info(f"[{datetime.now().isoformat()}] API Secret available: {bool(settings.X_API_SECRET)}")
        
        # Initialize with base config
        super().__init__(config, name="x_api")
        
        # Validate required settings
        if not all([settings.X_BEARER_TOKEN, settings.X_API_KEY, settings.X_API_SECRET]):
    ...
    async def process_response(self, response: ClientResponse) -> Union[Tweet, User]:
        """Process X API response with error handling."""
        logger.info(f"[{datetime.now().isoformat()}] Processing response: {response.status}")
        ...
        if "errors" in data:
            ...
            raise XAPIInvalidRequest(
                f"X API returned {len(errors)} errors",
                errors=errors
            )

        if "data" not in data:
            logger.warning("Empty response from X API")
            return None
