...
class AbstractFetcher(ABC, Generic[T]):
    ...
    async def handle_error(
        ...
        if isinstance(error, CircuitBreakerError):
            ...
            return FetchResult(
                status=FetchStatus.CIRCUIT_BROKEN,
                error=error
            )
    ...
    async def fetch(
        ...
        **kwargs
        ...
        cache_key = f"{method}:{url}:{str(sorted(params.items()) if params else '')}"
    ...
    def health_stats(self) -> Dict[str, any]:
        ...
        return {
            ...
            'circuit_break_rate': self._stats['circuit_breaks'] / total_requests
        }
