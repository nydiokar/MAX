...
class AnthropicClassifier(Classifier):
    def __init__(self, options: AnthropicClassifierOptions):
        ...
        self.system_prompt = (
            "You are an AI assistant."  # Add your system prompt here
        )
    ...
    async def process_request(
        ...
        try:
            ...
            if not is_tool_input(tool_use.input):
                raise ValueError(
                    "Tool input does not match expected structure"
                )

            intent_classifier_result = ClassifierResult(
                selected_agent=self.get_agent_by_id(
                    tool_use.input["selected_agent"]
                ),
