...
class FetcherAgent:
    ...
    async def _monitor_critical_accounts(self):
        ...
        async for tweet in self.twitter_client.stream_users(critical_accounts):
            await self._handle_tweet(tweet, DataProvenance.TWITTER_VERIFIED_ACCOUNT)

    async def _monitor_keywords(self):
        async for tweet in self.twitter_client.filter_stream(track=self.keyword_patterns):
            if await self._validate_tweet_source(tweet):
                await self._handle_tweet(tweet, DataProvenance.TWITTER_KEYWORD_MENTION)

    async def _validate_tweet_source(self, tweet: Dict[str, Any]) -> bool:
        return (
            tweet['user']['followers_count'] >= self.config.min_follower_count and
            (datetime.now() - tweet['user']['created_at']).days >= self.config.min_account_age_days and
            tweet['public_metrics']['like_count'] >= self.config.engagement_thresholds['likes']
        )

    async def _handle_tweet(self, tweet: Dict[str, Any], provenance: DataProvenance):
